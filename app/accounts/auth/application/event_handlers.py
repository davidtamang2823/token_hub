...
from accounts.auth.infrastructure.tasks import send_password_reset_email_task

@event_bus.subscribe(EventTypes.USER_RESEND_VERIFICATION)
...
@event_bus.subscribe(EventTypes.USER_PASSWORD_RESET_REQUEST)
def password_reset_request_handler(event: PasswordResetVerificationEvent):
    payload = event.to_dict()
    send_password_reset_email_task.apply_async(args=[payload])